...
var resizePizzas = function(size) {
  ...
  function determineDx (elem, size) {
	...
	// Moved multiple reference calls to `radonPizzaContainer` class into one variable.
	...
	if (!changingSizes) {
		window.requestAnimationFrame(function() { changePizzaSizes(size); });
		changingSizes = true;
	}
...
/*
Revised the hidden strategy to hide/reveal excess pizzas by applying the event listener
`onmouseover`/`onmouseout` on the slider bar. 23 pizzas should be enough for most screen sizes.
*/
...
sliderNode.onmouseover = function () { short_za_array(randomPizzaArray); };
sliderNode.onmouseout = function () { expand_za_array(randomPizzaArray); };
...
/*
The strategy here is to have the sliding pizzas bg rendered and animated by the canvas
element, which is set in its own layer. This vastly improves the frame rate because we
are no longer abusing the DOM with repeated reflows and repaints. All bg effects take
place in a single element in its own layer!
*/
...
// Calls `updatePositions` when the user scrolls the page. Using `rAF`
// ensures optimum rendering performance from the browser.
